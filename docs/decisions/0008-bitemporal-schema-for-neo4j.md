...
Epic [#97](https://github.com/100rd/Omniscience/issues/97) delivered every wave of this ADR over PRs #147 through #175. Final status flip mirrors how ADR-0005 and ADR-0006 flipped after [#105](https://github.com/100rd/Omniscience/issues/105). The PRs that contributed:

- **Wave 2 — schema migration** ([#130](https://github.com/100rd/Omniscience/issues/130), PR [#147](https://github.com/100rd/Omniscience/pull/147)): added the §4 constraint and index DDL to `_BOOTSTRAP_STATEMENTS` (composite on `workspace_id` first), introduced `_BITEMPORAL_BACKFILL_STATEMENTS` for legacy data, gated the new write path behind `GRAPH_BITEMPORAL=disabled` default. Three-phase backfill (entity props → `:EntityState` materialization → edge props), idempotent, workspace-scoped.
- **Wave 2 — writer versioning** ([#131](https://github.com/100rd/Omniscience/issues/131), PR [#156](https://github.com/100rd/Omniscience/pull/156)): writer produces a new `:EntityState` + `[:HAD_STATE]` chain on every state change; SHA-256 fingerprint over versioned fields drives state-change detection; one Cypher transaction per identity update (atomicity proven by mid-tx-failure rollback test).
- **Wave 3 step 1 — GraphStore `as_of`** ([#132](https://github.com/100rd/Omniscience/issues/132), PR [#170](https://github.com/100rd/Omniscience/pull/170)): two-template strategy on the Neo4j adapter (`as_of=None` reads the `:Entity` mirror; `as_of=T` traverses `:EntityState` via the `(workspace_id, valid_from, valid_to)` index). Open-closed boundary tested at microsecond resolution. `EntityNodeView` / `GraphEdgeView` gained optional `valid_from` / `valid_to` / `recorded_at`.
- **Wave 3 step 2 — MCP + REST `as_of`** ([#133](https://github.com/100rd/Omniscience/issues/133), PR [#173](https://github.com/100rd/Omniscience/pull/173)): user-facing `as_of` on `search`, `get_entity`, `get_related_entities`. Naive datetimes rejected with `400 INVALID_TIMEZONE`; pre-history returns `meta.degraded_response = "as_of_before_recorded_history"`; `effective_as_of` echoed back. New `as_of_kind` histogram label. `resolve_incident` deferred to [#153](https://github.com/100rd/Omniscience/issues/153).
- **Wave 3 step 3 — Qdrant payload + `as_of` filter** ([#134](https://github.com/100rd/Omniscience/issues/134), PR [#174](https://github.com/100rd/Omniscience/pull/174)): writer populates `valid_from` / `valid_to` / `recorded_at` on every chunk; payload indexes on the new fields; `must`-clause `as_of` predicate via the sanctioned `qdrant_filters.QdrantFilterBuilder.with_as_of(...)` builder; GraphRAG composer threads `as_of` to BOTH stores. `as_of=None` byte-identical to pre-#134 wire shape.
- **Wave 5 — end-dated tombstones** ([#137](https://github.com/100rd/Omniscience/issues/137), PR [#171](https://github.com/100rd/Omniscience/pull/171)): `_END_DATE_BY_SOURCE_CYPHER` and `_END_DATE_TOMBSTONED_CYPHER` replace the legacy hard-delete templates under `GRAPH_BITEMPORAL=enabled`; writer's `IndexWriter.upsert_graph(..., snapshot_at=...)` end-dates entities and edges absent from a new snapshot at the snapshot's clock. Qdrant gains `end_date_chunks(...)` for the same shape on the vector side.
- **Wave 6 — validation gate** ([#138](https://github.com/100rd/Omniscience/issues/138), PR [#175](https://github.com/100rd/Omniscience/pull/175)): the property-test suite at `tests/property/test_bitemporal_contract.py` proves §1, §3, §5, §6, §9 hold over `hypothesis`-generated multi-workspace fixtures (120 examples per property). **The cross-workspace-isolation property carries forward the [#117](https://github.com/100rd/Omniscience/issues/117) / [#119](https://github.com/100rd/Omniscience/issues/119) ACL invariant into the bitemporal dimension and is the P0 security gate that authorised this ADR's flip to Implemented.** Performance budgets are exercised in two tiers: a CI sanity floor on a 10k-node fixture and an opt-in perf-lab fixture (1M nodes / 1M edges) gated on `OMNISCIENCE_RUN_BITEMPORAL_PERF=1`.

### Wave 3 step 2 — MCP + REST `as_of` parameter ([#133](https://github.com/100rd/Omniscience/issues/133))

### Wave 3 step 2 — MCP + REST `as_of` parameter ([#133](https://github.com/100rd/Omniscience/issues/133))

Surfaces ADR-0008 §5's bitemporal predicate through the public MCP tools (`search`, `get_entity`, `get_related_entities`) and REST routes (`POST /api/v1/search`, `GET /api/v1/entities/{name}`, `GET /api/v1/entities/{name}/related`). Validation enforces UTC-only ISO-8601 datetimes (`invalid_timezone` error code on naive or non-UTC); responses carry `effective_as_of` echo-back; pre-history `as_of` returns an empty result with `meta.degraded_response = "as_of_before_recorded_history"`. The composer plumbs `as_of` to `GraphStore.find_related` only — Qdrant payload filter is the parallel sub-issue [#134](https://github.com/100rd/Omniscience/issues/134). `resolve_incident` is reserved for [#153](https://github.com/100rd/Omniscience/issues/153) and surfaces its own `as_of` natively when it lands. Telemetry: `omniscience_request_duration_seconds` carries the `as_of_kind` label (`none` / `past` / `future` / `pre_history`) so ops can see how often historical reads happen and at what cost. Cross-workspace isolation under `as_of` is a non-skippable contract test ([#117](https://github.com/100rd/Omniscience/issues/117) / [#119](https://github.com/100rd/Omniscience/issues/119) carry-forward in the bitemporal dimension; ADR-0008 §Consequences-security #4). See `docs/api/mcp.md` and `docs/api/rest.md` for the user-facing contract.

### Wave 2 of epic #99 — `resolve_incident` joined the `as_of`-aware tools ([#153](https://github.com/100rd/Omniscience/issues/153))

The MCP / REST `resolve_incident` tool composes the four Wave-1 incident sources (alerts, GitHub PR, Slack, OTel) into a recommendation bundle. It accepts `as_of` natively per the contract reserved in `#133` above and forwards the parameter to both `GraphStore.get_entity` (alert seed resolution) and `GraphStore.find_related` (BFS traversal to target resource / responsible PR / Slack threads). The forcing-function test `test_resolve_incident_not_present_yet` (added with #173) is replaced by a positive contract test in `tests/test_mcp_as_of.py::TestMcpToolSchemaContract::test_resolve_incident_schema_has_as_of` that asserts the FastMCP tool registry exposes `as_of` on `resolve_incident`. The same `omniscience_request_duration_seconds` histogram with `tool="resolve_incident"` records per-request wall-clock and the `as_of_kind` label. Cross-workspace ACL: a foreign workspace's `alert_id` returns `alert_not_found` (404) — the same response a non-existent alert would produce — so existence is never leaked. The v0.1 confidence-score heuristic is a deterministic placeholder per the architect memo on epic #99; the calibrated model lands in [#155](https://github.com/100rd/Omniscience/issues/155).

### Wave 3 step 3 — Qdrant bitemporal payload + `as_of` filter ([#134](https://github.com/100rd/Omniscience/issues/134))

Closes the cross-store `as_of` story. The Qdrant payload now carries `valid_from`, `valid_to`, `recorded_at` on every chunk upsert (defaults: `recorded_at = now()`, `valid_from = recorded_at`, `valid_to = NULL` per ADR-0008 §1 sentinel), and `valid_from` / `valid_to` are payload-indexed alongside `recorded_at` so the §5 canonical predicate is index-backed. Every Qdrant read method (`search`, `count`, `count_chunks`, `count_chunks_by_source`) accepts `as_of: datetime | None = None`; when supplied, the predicate `valid_from <= $as_of AND ($as_of < valid_to OR valid_to IS NULL)` is layered onto the workspace-scoped filter as additional `must` clauses via `QdrantFilterBuilder.with_as_of` — the workspace must-clause is never replaced (ADR-0006 §ACL carry-forward, mandatory cross-workspace isolation contract test). `as_of=None` is byte-identical to the pre-#134 wire shape — current-state reads do not pay the predicate cost. The GraphRAG composer threads `as_of` to BOTH the graph step (from #133) AND the vector step (NEW) so a v3 graph entity at `as_of=T1` returns chunks linked to the v1 chunk version that was valid at T1 — the cross-store consistency contract from §6. Telemetry adds `omniscience_qdrant_as_of_filtered_total{method}` so ops can see which Qdrant read paths exercise the bitemporal filter. Boundary semantics match §1's open-closed contract: `as_of == valid_from` is included (`lte`), `as_of == valid_to` is excluded (strict `gt`).

### Wave 6 — bitemporal contract validation gate ([#138](https://github.com/100rd/Omniscience/issues/138))

The property-test suite at `tests/property/test_bitemporal_contract.py` is the holistic validation gate for this ADR. Eight `hypothesis`-driven properties cover §5 (open-closed predicate), §3 (edge endpoint coupling), §6 (cross-store consistency with ADR-0006), and §9 (identity stability) — running at `max_examples=120` per property over a multi-workspace simulator with overlapping entity names and overlapping `valid_*` ranges (`tests/property/_simulator.py`, `tests/property/_strategies.py`). Property #7 — cross-workspace isolation under all `as_of` values — is the P0 security gate carried forward from [#117](https://github.com/100rd/Omniscience/issues/117) / [#119](https://github.com/100rd/Omniscience/issues/119) / [#173](https://github.com/100rd/Omniscience/issues/173) / [#174](https://github.com/100rd/Omniscience/issues/174); any failure blocks the ADR flip. The simulator-first approach (matching the precedent set by [#171](https://github.com/100rd/Omniscience/issues/171)'s `test_property_no_data_loss_across_end_dating`) keeps CI deterministic and fast; live-mode counterparts on real Neo4j+Qdrant testcontainers are gated behind `OMNISCIENCE_RUN_NEO4J_CONTRACT_TESTS=1` and `OMNISCIENCE_RUN_QDRANT_CONTRACT_TESTS=1`. The performance regression suite at `benchmarks/test_bitemporal_perf.py` carries a CI sanity floor (10k-node fixture) and a perf-lab opt-in (1M-node fixture, `OMNISCIENCE_RUN_BITEMPORAL_PERF=1`); see `tests/property/README.md` for the matrix. With #138 green, this ADR is eligible for the status flip in [#139](https://github.com/100rd/Omniscience/issues/139).
