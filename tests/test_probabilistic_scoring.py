...
def test_calculate_probabilistic_confidence() -> None:
    now = datetime.now(UTC)
    # depth=1, age=0 -> confidence should equal source reliability
    assert calculate_probabilistic_confidence(source="git", valid_from=now, depth=1, as_of=now) == 0.95
    
    # depth=2, age=0 -> 0.95 * e^(-0.15 * 1)
    expected = 0.95 * math.exp(-0.15)
    result = calculate_probabilistic_confidence(source="git", valid_from=now, depth=2, as_of=now)
    assert pytest.approx(result, abs=1e-5) == expected


def test_calculate_probabilistic_impact() -> None:
    now = datetime.now(UTC)
    # centrality=0 -> baseline topo=0.5. depth=1 -> depth decay=1.0. source reliability for slack=0.6. age=0 -> 1.0.
    # Expected: 0.5 * 1.0 * 0.6 * 1.0 = 0.3
    result = calculate_probabilistic_impact(source="slack", valid_from=now, depth=1, centrality=0.0, as_of=now)
    ...
    result = calculate_probabilistic_impact(source="git", valid_from=now, depth=1, centrality=10.0, as_of=now)
    assert pytest.approx(result, abs=1e-5) == expected
...
def test_calculate_probabilistic_incident_confidence() -> None:
    ...
    res = calculate_probabilistic_incident_confidence(alert=alert, classified=classified, max_depth=3, as_of=now)
