...
# Known model dimensions for OpenAI embedding models
...
class OpenAIEmbeddingProvider:
    ...
    Supports ``text-embedding-3-small`` (1536-d) and
    ...
    Args:
        ...
        base_url: API base URL (override for proxies / Azure OpenAI).
    ...
    async def close(self) -> None:
        ...
        log.debug("openai_client_closed", model=self._model)
    ...
    async def _post_embed(self, batch: list[str]) -> list[list[float]]:
        """POST one batch to the OpenAI /v1/embeddings endpoint."""
        payload: dict[str, Any] = {"model": self._model, "input": batch}
        log.debug("openai_embed_request", model=self._model, batch_size=len(batch))
        ...
        log.debug("openai_embed_ok", model=self._model, count=len(vectors))
        return vectors
